...
<%
	...
	ruta 			= Request("ruta") 
	    ...
	    Dim DestinationPath
			DestinationPath = Server.mapPath("../Archivo/InformesAnexados") & "\" & IntId
	...
	If Request.ServerVariables("REQUEST_METHOD") = "POST" and trim(accion_archivo) <> "descarga" then
		...
		AbrirSCG()
		strSql = "EXEC Proc_Audita_Archivo 1, 5, "&trim(session("session_idusuario"))&",null, '"&trim(IntId)&"', '"&trim(archivo)&"', '',0 "
	...
	If not Obj_FSO.FolderExists(Server.mapPath("../Archivo/InformesAnexados") & "\" & IntId) = True Then ' verifica la existencia del archivo
		Obj_FSO.CreateFolder(Server.mapPath("../Archivo/InformesAnexados") & "\" & IntId) 
	End if
...
%>
	...
	$(document).ready(function(){
		$('.div_historial_eliminados').toggle(function(){
			...
			var IntId 		= $('#IntId').val()				
			var criterios 	="alea="+Math.random()+"&accion_ajax=mostrar_archivos_eliminados_informe_clientes&IntId="+IntId
	...
	function Refrescar(strTipo){
		...
		frmSend.action='Informe_Clientes.asp?strTipoPropiedad=' + strTipo;
		if (frmSend.strFormMode.value == 'Nuevo') {
			location.href="Informe_Clientes.asp?sintNuevo=1&strTipoPropiedad=" + strTipo;
			}
		else {
		 	frmSend.submit();
		}
	...
	function enviar(){
		...
		var criterios ="alea="+Math.random()+"&accion_ajax=verifica_informes_anexados&nombre_archivo="+archivo+"&IntId="+IntId
		$('#verifica_archivo').load('FuncionesAjax/verifica_archivo_ajax.asp', criterios, function(){
			...
			{
				frmSend.action = "Informe_Clientes.asp?archivo=1";
				frmSend.submit();
	...
	function bt_descargar(ruta){
			...
			frmSend.action = "Informe_Clientes.asp?ruta="+ruta+"&accion_archivo=descarga";
			frmSend.submit();
	}
	</SCRIPT>
...
<BODY BGCOLOR='FFFFFF'>
	<div class="titulo_informe">INFORMES ANEXADOS</div>	
<FORM name="frmSend" id="frmSend" onSubmit="return enviar(this)"  method="POST" enctype="multipart/form-data" action="Informe_Clientes.asp">
	...
	<%	AbrirSCG()
		...
		SQL_SEL = SQL_SEL & "WHERE car.activo =1 AND cod_cliente="&trim(IntId)&" AND origen = 5 "
	...
	<%if not rs_sql_sel.eof then%>	
		...
		<table class="tabla_archivos_cargados estilo_columnas">
			...
			%>
					...
					<td>
						<a onclick="bt_eliminar('<%=trim(rs_sql_sel("cod_cliente"))%>','<%=trim(rs_sql_sel("nombre_archivo"))%>','informe_cliente','<%=trim(rs_sql_sel("id_archivo"))%>')" href="#"><img border="0" src="../imagenes/icon_cruz_roja.jpg"></a>
					</td>

					<td>
					
						<a href="#" onclick="bt_descargar('../Archivo/InformesAnexados/<%=trim(rs_sql_sel("cod_cliente"))%>/<%=trim(rs_sql_sel("nombre_archivo"))%>')"><%=trim(rs_sql_sel("nombre_archivo"))%></a>	
...
<%
	...
	SQL_SEL = SQL_SEL & "WHERE activo =0 AND cod_cliente="&trim(IntId)&" AND origen = 5 "
	set rs_sql_sel = Conn.execute(SQL_SEL)
%>
	<div class="contenedor_historial_eliminados">
	
		<div class="div_historial_eliminados  fondo_boton_100">
			<span class="">&nbsp;Historial archivos eliminados (<%=trim(rs_sql_sel("cantidad"))%>)</span> <img  id="img_historial_eliminados" height="13" width="13" src="../Imagenes/flecha_abajo.png">
		</div>
