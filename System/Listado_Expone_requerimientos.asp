...
<%
	...
	'######################## TIPO MODULO filtra tipo gestion, en este caso esta 3 (casos objetados)
	sql_resumen = sql_resumen & " AND TIPO_MODULO = 3 "

	sql_resumen = sql_resumen & " AND ID_PROCESO IN (1,2,3)"
...
function CargaUsuarios(valor){
	...
	$('#filtrado_ejecutivo').load('FuncionesAjax/listado_expone_requerimientos_ajax.asp', criterios, function(){})
}
...
function consulta_resumen()
{
	...
	//alert(criterios)
	$('#refresca_resumen').load('FuncionesAjax/listado_expone_requerimientos_ajax.asp', criterios, function(){
...
function exportar()
{
	...
	location.href="exp_Listado_Expone_requerimientos.asp?alea="+Math.random()+"&accion_ajax=refresa_objetados&COD_CLIENTE="+COD_CLIENTE+"&CB_COBRANZA="+CB_COBRANZA+"&CMB_TIPO_GESTION="+CMB_TIPO_GESTION+"&CMB_ESTADO_PROCESO="+CMB_ESTADO_PROCESO+"&inicio="+TX_INICIO+"&termino="+TX_TERMINO+"&CB_EJECUTIVO="+CB_EJECUTIVO+"&inicia_contador=20&RUT_DEUDOR="+RUT_DEUDOR+"&HORA_CONSULTA="+HORA_CONSULTA+"&FECHA_CONSULTA="+TX_FECHA_CONSULTA+"&CH_CP_ADJUNTO="+CH_CP_ADJUNTO
...
function procesar(){
	...
	$('#proceso_casos').load('FuncionesAjax/listado_expone_requerimientos_ajax.asp', criterios, function(){
...
function busca_cuotas(intIdGestion, RUT_DEUDOR){
	...
	$(concat_refresca).load('FuncionesAjax/listado_expone_requerimientos_ajax.asp', criterios, function(){
		var strIDCuotas =$('#strIDCuotas').val()
		//alert(strIDCuotas)
		if(strIDCuotas!=""){

			var criterios ="alea="+Math.random()+"&accion_ajax=mostrar_todos_cuotas&rut="+RUT_DEUDOR+"&strCodCliente="+strCodCliente+"&ID_GESTION="+intIdGestion+"&strIDCuotas="+strIDCuotas+"&CH_TODOS_CUOTA=1&pagina_origen=casos_objetados"
...
function ventanaMas (URL){
	window.open(URL,"DATOS1","width=400, height=500, scrollbars=no, menubar=no, location=no, resizable=yes")
}
...
function bt_trae_cuotas_vista(ID_GESTION,RUT_DEUDOR, COD_CLIENTE){
	...
	$(concat_refresca).load('FuncionesAjax/listado_expone_requerimientos_ajax.asp', criterios, function(){
		...
		if(strIDCuotas!=""){
			window.location ="detalle_gestiones.asp?rut="+RUT_DEUDOR+"&cliente="+COD_CLIENTE+"&strIDCuotas="+strIDCuotas+"&strNuevaGestion=S&pagina_origen=casos_objetados"
...
<div class="titulo_informe">MÓDULO CASOS OBJETADOS</div>
<br>	
		  ...
		  <tr>
			...
			<td>
				...
				<SELECT NAME="CMB_TIPO_GESTION" id="CMB_TIPO_GESTION" style="background-color:#ccc;" multiple>
					<option value="3" SELECTED>CASO OBJETADO</option>
				</SELECT>
			</td>
			
			<td>
				<SELECT NAME="CMB_ESTADO_PROCESO" id="CMB_ESTADO_PROCESO" multiple style="width:100px;background-color:#ccc;">
					<option value="1" SELECTED>NO PROCESADO</option>
					<option value="2" SELECTED>NO RESPONDIDO</option>
