...
<%
	...
	strObTipoProcesoObj =" SELECTED"
	
	'response.write "Ftro_EstadoProcesoCasosObj=" & Ftro_EstadoProcesoCasosObj
	
	IF session("Ftro_EstadoProcesoCasosObj")="" THEN
	...
	ELSE
		...
		IF instr(session("Ftro_EstadoProcesoCasosObj"), "1")>0 then
			strObEstadoProcesoNP =" SELECTED "
		END IF 
		
		IF instr(session("Ftro_EstadoProcesoCasosObj"), "2")>0 then
			strObEstadoProcesoNR =" SELECTED "
		END IF 

		IF instr(session("Ftro_EstadoProcesoCasosObj"), "3")>0 then
...
function CargaUsuarios(valor){
	...
	$('#filtrado_ejecutivo').load('FuncionesAjax/casos_objetados_ajax.asp', criterios, function(){})
}
...
function consulta_detalle()
{
	...
	//alert(criterios)
	$('#refresca').load('FuncionesAjax/casos_objetados_ajax.asp', criterios, function(){})
}
...
function consulta_resumen()
{
	...
	//alert(criterios)
	$('#refresca_resumen').load('FuncionesAjax/casos_objetados_ajax.asp', criterios, function(){
...
function exportar()
{
	...
	location.href="exp_casos_objetados.asp?accion_ajax=refresa_objetados&COD_CLIENTE="+COD_CLIENTE+"&CB_COBRANZA="+CB_COBRANZA+"&CMB_TIPO_GESTION="+CMB_TIPO_GESTION+"&CMB_ESTADO_PROCESO="+CMB_ESTADO_PROCESO+"&inicio="+TX_INICIO+"&termino="+TX_TERMINO+"&CB_EJECUTIVO="+CB_EJECUTIVO+"&inicia_contador=20&RUT_DEUDOR="+RUT_DEUDOR+"&HORA_CONSULTA="+HORA_CONSULTA+"&FECHA_CONSULTA="+TX_FECHA_CONSULTA+"&CH_CP_ADJUNTO="+CH_CP_ADJUNTO
...
function procesar(){
	...
	$('#proceso_casos').load('FuncionesAjax/casos_objetados_ajax.asp', criterios, function(){
...
function busca_cuotas(intIdGestion, RUT_DEUDOR){
	...
	$(concat_refresca).load('FuncionesAjax/casos_objetados_ajax.asp', criterios, function(){
		  ...
		  <tr>
			...
			<td>
				...
				<SELECT NAME="CMB_TIPO_GESTION" id="CMB_TIPO_GESTION" style="background-color:#ccc;" multiple>
					<option value="1" <%=strObTipoProcesoObj%> >CASO OBJETADO</option>
				</SELECT>
			</td>
