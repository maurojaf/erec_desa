...
abrirscg()
strSql = 		  "SELECT G.RUT_DEUDOR,CU.NRO_DOC,CU.NRO_CUOTA, G.COD_CATEGORIA,G.COD_SUB_CATEGORIA,ISnull(G.COD_GESTION,0) as "
...
strSql = strSql & " convert(varchar(2),G.COD_CATEGORIA) + convert(varchar(2),G.COD_SUB_CATEGORIA) + convert(varchar(2),IsNull(G.COD_GESTION,0)) as COD_GESTION, G.ID_USUARIO , "
strSql = strSql & " (CASE WHEN ISNULL(DD.TELEFONO,'SIN FONO')='' THEN 'SIN FONO' ELSE ISNULL(DD.TELEFONO,'SIN FONO') END) AS TELEFONO_ASOCIADO,"
strSql = strSql & " TC.DESCRIPCION + ' ' + TS.DESCRIPCION + ' ' + TG.DESCRIPCION AS DESCRIPCION "

strSql = strSql & " FROM GESTIONES G, CUOTA CU,GESTIONES_CUOTA GC, GESTIONES_TIPO_CATEGORIA TC, GESTIONES_TIPO_SUBCATEGORIA TS, GESTIONES_TIPO_GESTION TG"

strSql = strSql & " WHERE G.ID_GESTION = GC.ID_GESTION"
strSql = strSql & " AND GC.ID_CUOTA = CU.ID_CUOTA"
strSql = strSql & " AND G.COD_CATEGORIA = TC.COD_CATEGORIA"
...
strSql = strSql & " AND G.ID_MEDIO_GESTION = DD.ID_TELEFONO "
...
strSql = strSql & " AND G.COD_CLIENTE = '" & intCOD_CLIENTE & "'"
...
strSql = strSql & "	group by G.RUT_DEUDOR,CU.NRO_DOC,CU.NRO_CUOTA, G.COD_CATEGORIA,G.COD_SUB_CATEGORIA,ISnull(G.COD_GESTION,0), "
strSql = strSql & "	G.FECHA_INGRESO,G.HORA_INGRESO,G.FECHA_COMPROMISO,G.OBSERVACIONES ,G.COD_GESTION, G.ID_USUARIO , DD.TELEFONO,  TC.DESCRIPCION + ' ' + TS.DESCRIPCION + ' ' + TG.DESCRIPCION "
strSql = strSql & " order by G.RUT_DEUDOR,G.COD_CATEGORIA,G.COD_SUB_CATEGORIA,ISnull(G.COD_GESTION,0), "
strSql = strSql & " G.FECHA_INGRESO,G.HORA_INGRESO,G.FECHA_COMPROMISO,G.OBSERVACIONES,G.ID_USUARIO , DD.TELEFONO "
...
<tr>
	...
	<td class="DatosDeudorTexto"><font class="TextoDatos"><%= rsges("NRO_DOC") %></font></td>
	<td class="DatosDeudorTexto"><font class="TextoDatos"><%= rsges("NRO_CUOTA") %></font></td>
