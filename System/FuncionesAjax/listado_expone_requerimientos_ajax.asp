...
elseif trim(accion_ajax)="refresa_objetados" then
	...
	session("Ftro_TipoGestionCasosObj") = strTipoGestion
	...
	sql_sel_casos = sql_sel_casos & " LEFT JOIN USUARIO U ON VV.ID_USUARIO_ASIG = U.ID_USUARIO   "
  	...
  	sql_sel_casos = sql_sel_casos & " WHERE VV.COD_CLIENTE ='"&TRIM(strCodCliente)&"' AND VV.TIPO_MODULO in ("&trim(strTipoGestion)&")"
	...
	do while not rs_casos_gestion.eof
		...
		if cint(intContador) <= cint(intInicioContador) then
			...
			<tr class="td_hover">
				...
				<td width="70"><%=trim(dtmFechaVenc)%></td>
...
elseif trim(accion_ajax)="refresca_resumen" then
	...
	sql_resumen = sql_resumen & " WHERE COD_CLIENTE ='"&TRIM(strCodCliente)&"' AND TIPO_MODULO in ("&trim(strTipoGestion)&")"
