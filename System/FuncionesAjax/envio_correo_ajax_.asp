...
if trim(accion_ajax)="envio_plan_pago" then
	...
	SQL_SEL = SQL_SEL & " WHERE COD_CLIENTE =  " & TRIM(session("ses_codcli")) & " AND COD_CORREO='"&trim(COD_CORREO)&"'"
...
com = Chr(34)   
str_concat = "<div style="&com&"text-align: RIGHT;"&com&"><img src="&com&session("ses_ruta_web")&"/Imagenes/Logos/"&session("ses_codcli")&"/Logo.jpg"&com&"></div>"
...
str_concat = str_concat & "</table>"
			...
			sql_sel ="Exec Proc_Envio_mail_deudor 'PLAN_PAGO','"&lcase(email)&"','"&trim(str_concat)&"','"&trim(session("session_idusuario"))&"' ,'"&trim(rut)&"','("&trim(cuotas_rut)&")','"&trim(session("ses_codcli"))&"','"&trim(CORREO_SALIENTE)&"','"&trim(adj_pdf)&"','"&trim(adj_excel)&"','"&trim(concat_con_copia)&"','"&trim(ASUNTO_CORREO)& " Rut: "&trim(rut)&" Nombre: "&trim(ucase(NOMBRE_DEUDOR))&"','"&trim(fecha_generar_documentos)&"','"&TRIM(session("ses_ruta_sitio"))&"'"	
...
elseif trim(accion_ajax)="crea_correo_electronico" then
	...
	sql_sel_email = sql_sel_email & " INNER JOIN DEUDOR d ON d.RUT_DEUDOR=de.RUT_DEUDOR AND D.COD_CLIENTE = '" & session("ses_codcli") & "'"
	...
	if not rs_email.eof then
		...
		if trim(correo_electronico)<>""	and not isnull(correo_electronico) then
			...
			SQL_SEL =" SELECT NOM_CORREO, ASUNTO_CORREO, CORREO_SALIENTE, CUERPO_CORREO, FIRMA, COD_CORREO "
			SQL_SEL = SQL_SEL & " FROM ENVIO_CORREO "
			SQL_SEL = SQL_SEL & " WHERE COD_CLIENTE =  " & TRIM(session("ses_codcli"))
						...
						<table class="tabla_contenido">
							...
							<tr>
								...
								<td class="contenido_envio" valign="top">
									<select name="con_copia" id="con_copia" multiple>
										<option selected value="<%=trim(correo_electronico)%>"><%=trim(lcase(correo_electronico))%></option>	
										<%if not rs_email_sup.eof then
											do while not rs_email_sup.eof	
										
												if trim(rs_email_sup("ID_USUARIO"))="372" then

													if trim(session("ses_codcli"))=1100 or trim(session("ses_codcli"))=1600 then
													correo_electronico_sup =rs_email_sup("correo_electronico")
													%>
													<option selected value="<%=trim(correo_electronico_sup)%>"><%=trim(lcase(correo_electronico_sup))%></option>
													<%
													else
													%>
													<option selected value="jaraya@llacruz.cl">jaraya@llacruz.cl</option>
													<%													
													end if
													...
													correo_electronico_sup =rs_email_sup("correo_electronico")
													%>
													<option value="<%=trim(correo_electronico_sup)%>"><%=trim(lcase(correo_electronico_sup))%></option>
													<%													
												end if
												
											rs_email_sup.movenext
											loop
										end if%>
							...
							<tr>
								...
								<td class="contenido_envio" valign="middle" >
									<input type="checkbox" name="adj_pdf" checked id="adj_pdf" value="1"><a href="#" onclick="bt_descargar('../Archivo/BibliotecaNotificacionDeudores/<%=trim(session("ses_codcli"))%>/<%=rut%>/<%=fecha_generar_documentos%>_detalle_deuda.pdf')">&nbsp;<img border="0" src="../Imagenes/icono_pdf.jpg" width="25" height="25" alt="" style="cursor:pointer;"> Detalle_deuda.pdf</a>
									&nbsp;&nbsp;
									<input type="checkbox" checked name="adj_excel" id="adj_excel" value="1"><a href="../Archivo/BibliotecaNotificacionDeudores/<%=trim(session("ses_codcli"))%>/<%=rut%>/<%=fecha_generar_documentos%>_detalle_deuda.csv">&nbsp;<img src="../Imagenes/icono_excel.jpg" border="0" width="25" height="25" alt=""> Detalle_deuda.csv</a>									
								</td>
							</tr>																						
							<tr>
...
elseif trim(accion_ajax)="visualiza_correo" then
	...
	if not RS_SEL.eof then	
		...
		<br>
		<div>
			<div class="imagens_email"><img src="../Imagenes/Logos/<%=session("ses_codcli")%>/Logo.jpg" alt=""></div>
			...
			<div class="comunicacion_email"><p>Comuníquese con su ejecutivo (a) <%=PonerPrimeraLetraEnMayuscula(nombres_usuario)&" "&PonerPrimeraLetraEnMayuscula(apellido_paterno)%> al teléfono fijo 28978900 (<%=des_anexo%> opción cobranzas) o correo electrónico <%=trim(lcase(correo_electronico))%></p></div>
