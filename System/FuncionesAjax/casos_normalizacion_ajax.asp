...
elseif trim(accion_ajax)="refresa_normalizados" then
	...
	session("Ftro_EstadoProcesoCasosNorm") = strEstadoProceso
	session("Ftro_EjecAsigCasosNorm") = strEjecutivo
	session("Ftro_TipoGestionCasosNorm") = strTipoGestion
	session("Ftro_Cp_Adjunto") = CH_CP_ADJUNTO
	
	'Response.write "valor : " & CH_CP_ADJUNTO
	''response.end
	
	sql_sel_casos = " SELECT TIPO_GESTION = (CASE WHEN TIPO_MODULO = 2"
	sql_sel_casos = sql_sel_casos & " 							THEN 'INDICA QUE PAGO'" 
	sql_sel_casos = sql_sel_casos & " 							WHEN (VV.TIPO_MODULO = 1 OR VV.TIPO_MODULO = 11) AND (VV.FORMA_PAGO = 'TR' OR VV.FORMA_PAGO = 'DP')"
	...
	sql_sel_casos = sql_sel_casos & " FECHA_GESTION = (CASE WHEN TIPO_MODULO = 2 THEN ISNULL(CONVERT(VARCHAR(10),VV.FECHA_PAGO,103),'NO ESPEC.')"
	sql_sel_casos = sql_sel_casos & " 							  WHEN (VV.TIPO_MODULO = 1 OR VV.TIPO_MODULO = 11) AND (VV.FORMA_PAGO = 'TR' OR VV.FORMA_PAGO = 'DP') THEN ISNULL(CONVERT(VARCHAR(10),VV.FECHA_COMPROMISO,103),'NO ESPEC.')"
	sql_sel_casos = sql_sel_casos & " 						      ELSE ISNULL(CONVERT(VARCHAR(10),VV.FECHA_AGENDAMIENTO_GESTION,103),'NO ESPEC.')"
	sql_sel_casos = sql_sel_casos & " 					     END),"
	...
	sql_sel_casos = sql_sel_casos & " LEFT JOIN USUARIO U ON VV.ID_USUARIO_ASIG = U.ID_USUARIO   "
  	...
  	sql_sel_casos = sql_sel_casos & " WHERE VV.COD_CLIENTE ='"&TRIM(strCodCliente)&"' AND (1=2"
		
	If instr((trim(strTipoGestion)), "1") > 0 then
	
		sql_sel_casos = sql_sel_casos & " OR (VV.TIPO_MODULO = 2)"
		
	end If 

	If instr((trim(strTipoGestion)), "2") > 0 then
	
		sql_sel_casos = sql_sel_casos & " OR ((VV.TIPO_MODULO = 1 OR VV.TIPO_MODULO = 11) AND VV.FORMA_PAGO IN ('TR','DP') AND DATEDIFF(DAY,VV.FECHA_COMPROMISO,GETDATE()) >= 0)"
		
	end If 
	
	If instr((trim(strTipoGestion)), "3") > 0 then
	
		sql_sel_casos = sql_sel_casos & " OR (VV.TIPO_MODULO = 6 AND AGEND_VENC = 1)"
		
	end If 

		sql_sel_casos = sql_sel_casos & " )"
	...
	sql_sel_casos = sql_sel_casos & " VV.FECHA_PAGO,"
	sql_sel_casos = sql_sel_casos & " VV.FECHA_AGENDAMIENTO_GESTION, "
	sql_sel_casos = sql_sel_casos & " VV.HORA_INGRESO,  "
	sql_sel_casos = sql_sel_casos & " VV.MONTO_GESTION, "
	sql_sel_casos = sql_sel_casos & " VV.FECHA_AGENDAMIENTO,"
	sql_sel_casos = sql_sel_casos & " VV.FECHA_COMPROMISO,"
	...
	sql_sel_casos = sql_sel_casos & " U.LOGIN,   " 
	sql_sel_casos = sql_sel_casos & " VV.PROCESO , "
	sql_sel_casos = sql_sel_casos & " VV.ID_PROCESO, VV.OBSERVACION_CONSULTA"
...
%>
	...
	do while not rs_casos_gestion.eof
		...
		if cint(intContador) <= cint(intInicioContador) then
			...
			strtipoGestion 			=rs_casos_gestion("TIPO_GESTION")
			...
			<tr class="td_hover">
				...
				<td width="130"><%=mid(trim(strtipoGestion),1,20)%></td>
				...
				<td width="30" align = "left" title="<%=strEjecutivo%>">
					<%=mid(trim(strEjecutivo),1,10)%>
				</td>				
...
<%
	...
	if err then
		...
		IntTotalGeneralDocNoRespondido = 0
		IntTotalGeneralSaldoNoRespondido = 0
		IntTotalGeneralCasosNoRespondido = 0

		intTotalGeneralDocNoProcesado = 0
		intTotalGeneralSaldoNoProcesado = 0
		intTotalGeneralCasosNoProcesado = 0

		intTotalGeneralDocNoConsulta = 0
		intTotalGeneralSaldoNoConsulta = 0
		intTotalGeneralCasosNoConsulta = 0
